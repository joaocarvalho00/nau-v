// NauV — 5-Stage Pipelined RV32I Core with Branch Predictor
//
// Module name is `core` (identical interface to the single-cycle core.sv and
// core_pipeline.sv) so that all testbenches and synthesis scripts work
// unchanged.  The Makefile selects which file to compile:
//   PIPELINE=0              → core.sv
//   PIPELINE=1              → core_pipeline.sv
//   PIPELINE=1 BRANCH_PREDICT=1 → core_pipeline_bp.sv
//
// This variant adds a 64-entry direct-mapped BTB (branch_predictor.sv) with
// 2-bit saturating counters.  When the BTB predicts taken, the PC is
// redirected in the same cycle as the fetch, giving a 0-cycle penalty for
// correctly predicted taken branches.  Mispredictions still incur the
// 2-cycle flush penalty (IF/ID and ID/EX are flushed), but these become rare
// once the predictor warms up.
//
// Branch penalty table:
//   Correctly predicted taken  : 0 cycles
//   Correctly predicted NT     : 0 cycles
//   Misprediction (any)        : 2 cycles  (flush IF/ID and ID/EX)
//
// Pipeline stages:
//   IF  — fetch instruction from imem, advance PC (BTB redirect if predicted)
//   ID  — decode instruction, read register file
//   EX  — execute ALU operation, resolve branch/jump, detect misprediction
...
//   · Branch/jump flush: flushed only on MISPREDICTION (ex_mispred), not on
//                        every taken branch/jump as in core_pipeline.sv.
//
...
module core (
    ...
    logic        if_id_bp_taken;   // BTB prediction that was made for this instr
    logic [31:0] if_id_bp_target;  // BTB predicted target for this instr
    ...
    logic        id_ex_auipc;
    logic        id_ex_bp_taken;   // BTB prediction carried to EX for comparison
    logic [31:0] id_ex_bp_target;  // BTB predicted target carried to EX
    ...
    // =========================================================================
    logic        bp_raw_taken;
    logic [31:0] bp_raw_target;

    // Only predict for branch/jump opcodes fetched this cycle
    logic [6:0] if_opcode;
    logic        if_is_ctrl;
    assign if_opcode  = if_instr[6:0];
    assign if_is_ctrl = (if_opcode == 7'b1100011) ||  // BRANCH
                        (if_opcode == 7'b1101111) ||  // JAL
                        (if_opcode == 7'b1100111);    // JALR

    logic        bp_pred_taken;
    logic [31:0] bp_pred_target;
    assign bp_pred_taken  = bp_raw_taken && if_is_ctrl;
    assign bp_pred_target = bp_raw_target;

    // BTB update: fire when a branch/jump resolves in EX
    logic bp_upd_en;
    assign bp_upd_en = id_ex_branch || id_ex_jump || id_ex_jalr;

    branch_predictor #(.ENTRIES(64)) u_bp (
        .clk              (clk),
        .rst              (rst),
        .bp_fetch_pc      (if_pc),
        .bp_pred_taken    (bp_raw_taken),
        .bp_pred_target   (bp_raw_target),
        .bp_update_en     (bp_upd_en),
        .bp_update_pc     (id_ex_pc),
        .bp_update_taken  (ex_pc_sel),
        .bp_update_target (ex_pc_target)
    );

    // =========================================================================
    // Misprediction detection
    // Fires when the instruction leaving EX is a branch/jump and the
    // BTB prediction made when it was fetched was wrong.
    // =========================================================================
    logic        ex_mispred;
    logic [31:0] ex_correct_pc;

    always_comb begin
        ex_mispred = (id_ex_branch || id_ex_jump || id_ex_jalr) &&
                     ((id_ex_bp_taken != ex_pc_sel) ||
                      (id_ex_bp_taken && ex_pc_sel &&
                       id_ex_bp_target != ex_pc_target));
        ex_correct_pc = ex_pc_sel ? ex_pc_target : id_ex_pc_plus4;
    end

    // =========================================================================
    // PC mux: priority 1 = misprediction correction, priority 2 = BTB prediction
    // =========================================================================
    logic        pc_sel_comb;
    logic [31:0] pc_target_comb;
    always_comb begin
        if (ex_mispred) begin
            pc_sel_comb    = 1'b1;
            pc_target_comb = ex_correct_pc;
        end else if (bp_pred_taken) begin
            pc_sel_comb    = 1'b1;
            pc_target_comb = bp_pred_target;
        end else begin
            pc_sel_comb    = 1'b0;
            pc_target_comb = 32'h0;
        end
    end
    ...
    // IF/ID Pipeline Register
    // =========================================================================
    // Flush only on misprediction — correctly predicted branches/jumps don't flush
    logic flush_if_id, flush_id_ex;
    assign flush_if_id = ex_mispred;
    assign flush_id_ex = ex_mispred;
    ...
    always_ff @(posedge clk) begin
        if (rst || flush_if_id) begin
            ...
            if_id_bp_taken  <= 1'b0;
            if_id_bp_target <= 32'h0;
        end else if (!stall) begin
            if_id_pc        <= if_pc;
            if_id_pc_plus4  <= if_pc_plus4;
            if_id_instr     <= if_instr;
            if_id_bp_taken  <= bp_pred_taken;
            if_id_bp_target <= bp_pred_target;
        end
    ...
    // =========================================================================
    // Hazard Unit
    // Flush is handled by ex_mispred, not ex_pc_sel.  Pass 1'b0 so the hazard
    // unit only handles load-use stalls; its flush outputs are left unconnected.
    // =========================================================================
    ...
    hazard_unit u_hazard (
        ...
        .ex_pc_sel     (1'b0),    // flush handled by ex_mispred, not here
        .stall         (stall),
        .flush_if_id   (),        // unused — ex_mispred drives flush_if_id
        .flush_id_ex   ()         // unused — ex_mispred drives flush_id_ex
    );
    ...
    always_ff @(posedge clk) begin
        if (rst || flush_id_ex || stall) begin
            ...
            id_ex_bp_taken    <= if_id_bp_taken;
            id_ex_bp_target   <= if_id_bp_target;
        end
    end
    ...
    // =========================================================================
    // Branch-predictor accuracy counters (simulation only)
    //
    // bp_upd_en fires every cycle that a branch or jump resolves in EX.
    // ex_mispred fires whenever that prediction was wrong.
    //
    // At simulation end a single summary line is printed:
    //   [BP STATS] total=N mispred=M accuracy=PP.PP%
    // This line is parsed by run_dhrystone.sh and tb_prog to report accuracy.
    // =========================================================================
    longint unsigned _bp_total;
    longint unsigned _bp_mispred;
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            if (bp_upd_en)  _bp_total   <= _bp_total   + 1;
            if (ex_mispred) _bp_mispred <= _bp_mispred + 1;
        end
    end

    final begin
        if (_bp_total > 0) begin
            automatic real accuracy_pct =
                100.0 * real'(_bp_total - _bp_mispred) / real'(_bp_total);
            $display("  [BP STATS]  total=%0d  mispred=%0d  accuracy=%.2f%%",
                     _bp_total, _bp_mispred, accuracy_pct);
        end
    end
